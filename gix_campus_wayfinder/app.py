...
def _category_badge_markdown(category: str) -> str:
    colors = {
        "Fabrication": "#1f77b4",
        "Study": "#2ca02c",
        "Printing": "#9467bd",
    ...
    return (
        ...
        f'border-radius:0.4rem;font-size:0.85rem;">{category}</span>'
    )
...
def main():
    ...
    st.metric("Matching resources", len(matches))
    ...
    if len(matches) == 0:
        ...
        for res in matches:
            with st.container():
                ...
                st.markdown(_category_badge_markdown(res["category"]), unsafe_allow_html=True)
